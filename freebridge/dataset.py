...
class AFHQ(Dataset):
    ...
    def __init__(self, resize, animals, split="train"):
        ...
        transform_list = ([transforms.RandomHorizontalFlip()] if split == "train" else [])
...
class GaussianMM:
    ...
    def logprob(self, x):
        logprobs = normal_logprob(x.unsqueeze(1), self.centers.unsqueeze(0), self.logstd)
...
# -------------------------
# BBBC021 latent samplers (callable samplers, NOT DataLoader)
# -------------------------
class _BBBC021LatentSampler:
    """
    sampler(n) -> (n, D) CPU tensor, supports .set_generator().
    """
    def __init__(self, np_array: np.ndarray, generator: torch.Generator | None = None):
        assert np_array.ndim == 2
        self.data = np_array.astype(np.float32, copy=False)
    ...
    def __call__(self, n: int) -> torch.Tensor:
        if self.generator is None:
            ...
            idx = torch.randint(0, self.N, (n,), generator=self.generator)
        return torch.from_numpy(self.data[idx.numpy()]).float()



def _resolve_data_path(path: str) -> str:
    """Resolve a data path against the repo root, so it works even when Hydra
    changes the working directory (hydra.job.chdir)."""
    from pathlib import Path
    p = Path(path)
    if p.is_absolute():
        return str(p)
    return str(get_repo_path() / p)

def _load_pair(src_path: str, tgt_path: str) -> tuple[np.ndarray, np.ndarray]:
    src = np.load(_resolve_data_path(src_path))
    tgt = np.load(_resolve_data_path(tgt_path))
    if src.ndim != 2 or tgt.ndim != 2:
        raise ValueError(f"Expected 2D latent arrays, got src {src.shape}, tgt {tgt.shape}")
    if src.shape[1] != tgt.shape[1]:
        raise ValueError(f"Latent dim mismatch: src {src.shape}, tgt {tgt.shape}")
    # NOTE: source and target are independent (unpaired) populations; we do NOT
    # truncate to a common length. Random intra-batch pairing happens in the
    # sampler and is only an optimization device.
    if src.shape[0] == 0 or tgt.shape[0] == 0:
        raise ValueError(f"Empty endpoint array: src {src.shape}, tgt {tgt.shape}")
    return src.astype(np.float32, copy=False), tgt.astype(np.float32, copy=False)


def get_dist_boundary(cfg):
    """
    Return p0, p1, p0_val, p1_val as callable samplers.

    This release wires the BBBC021 latent-transport task. Auxiliary toy/image tasks
    are not wired in the public FreeBridge release.
    """
    if str(cfg.prob.name) != "bbbc021":
        raise NotImplementedError(
            f"get_dist_boundary only supports prob.name='bbbc021' in this release; "
            f"got '{cfg.prob.name}'. Only the BBBC021 task is wired in this release."
        )

    src_tr, tgt_tr = _load_pair(cfg.data.src_train, cfg.data.tgt_train)
    D = src_tr.shape[1]
    if int(cfg.dim) != D:
        raise ValueError(f"cfg.dim={cfg.dim} but latent arrays have D={D}")
    if src_va.shape[1] != D or tgt_va.shape[1] != D:
        raise ValueError(f"Val latent dim mismatch: train D={D}, src_val={src_va.shape}, tgt_val={tgt_va.shape}")
    src_va, tgt_va = _load_pair(cfg.data.src_val, cfg.data.tgt_val)

    p0 = _BBBC021LatentSampler(src_tr)
    p1 = _BBBC021LatentSampler(tgt_tr)
    p0_val = _BBBC021LatentSampler(src_va)
    p1_val = _BBBC021LatentSampler(tgt_va)
