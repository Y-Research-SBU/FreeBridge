...
def sdeint(
    ...
    if nfe is not None and log_steps is not None:
        if log_steps > nfe + 1:
            raise ValueError(f"log_steps={log_steps} must be <= nfe+1={nfe+1}")
        if log_steps < 2:
            raise ValueError(f"log_steps must be >= 2, got {log_steps}")
