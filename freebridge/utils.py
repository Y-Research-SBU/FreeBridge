...
def get_job_directory(file_or_checkpoint: Union[str, Dict[str, Any]]) -> str:
    ...
    hydra_files = []
    while True:
        candidate = os.path.join(directory, ".hydra", "config.yaml")
        if os.path.exists(candidate):
            hydra_files = [candidate]
            break
        parent = os.path.dirname(directory)
        if parent == directory:   # reached filesystem root
            raise ValueError(f"Failed to find .hydra/config.yaml above: {file}")
        directory = parent
    job_dir = os.path.dirname(os.path.dirname(hydra_files[0]))
    return job_dir


def restore_model(checkpoint, pl_name="freebridge.pl_model", device=None):
    ...
    model = pl_module.FreeBridgeLitModule(cfg, p0, p1, p0_val, p1_val)
